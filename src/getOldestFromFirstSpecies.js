...
function getOldestFromFirstSpecies(id) {
  ...
  const funcionarios = data.employees.find((element) => element.id === id).responsibleFor[0];
  ...
  const informações = data.species.find((element) => element.id === funcionarios).residents;
